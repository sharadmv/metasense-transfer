...
def level1(out_dir, experiment_dir):
    ...
    for e_dir in tqdm.tqdm(fs.ls(experiment_dir)):
        e_dir = Path(e_dir)
        with fs.open(str(e_dir / 'models' / 'model.pkl'), 'rb') as fp:
        ...
        experiment = all_experiments - frozenset(map(process, frozenset(name.split("-"))))
