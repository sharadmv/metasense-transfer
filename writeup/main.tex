...
Recommendations for Density Plots:

- Figure: three rows (one for each model), in each row add error density plots for O3 for Abs Hum; make the max/min on all axes the same to make comparisons easier, make color coding consistent
- Placement: I would recommend placing this figure and the disc. before the target plots (then the flow can be (1) overall results, (2) digging into why we are seeing what we're seeing, (3) what does it mean and what are the implications (error vs. bias disc.)

Observation to work into the text: 
\begin{itemize}
    \item The results are in line with what we would expect for the MLR: training at Donovan and testing at El Cajon (at humidites that were included in the training data at Donovan), we see worse performance at Shafter than El Cajon, at humidities where extrapolation is not occurring. This suggests that background conditions play a factor (other pollutants, different background pollutant profiles, etc.) as well as environmental, because we would expect more similar background conditions between the two SD sites.
    \item When we train at El Cajon and test at Donovan (for MLR) again we see what we would expect - extrapolation in humidity values (environmental conditions) results in enhanced error (panel b, elevated error at high and low humidities). 
    \item Finally, training at Shafter seems to result in comparable errors at the two San Diego sites. 
    \item When we look at the RF results the story changes, across the board, no matter where we train the errors at the other two sites seem comparable, regardless of whether we saw those humidity values at the training site. One way to look at this is that the errors at the training site are not very indicative of errors at a new site - even in the same city. This seems to be more evidence of over-fitting.
    \item NEXT: How dos this look for NN-2? Are the results similar to the RF or MLR? Or is it somewhere in between? 
...
As low-cost sensor studies move from understanding sensor signal performance to how this performance is affected by moving sensors to new sampling locations or utilizing them in new applications, it’s important that results are translated into best practices to support the collection of usable high quality data. This is particularly important given the interest in sensors by communities based organizations and citizen scientists. While there are limitations to the work presented here, given that we only examined electrochemical O3 and NO2 sensors and the sampling sites were limited to three in California. There is still a need to continue examining different sensors and to learn more about the consistency of results across locations and situations. 
...
\begin{itemize}
    \item When conducting a “field calibration” (or co-locating your sensors with trusted reference monitors), co-locating your sensors at multiple reference locations will likely improve the reliability of the data from your sensors when they are moved to the field sites.
    ...
    \item When conducting a field calibration, placing sensors at different co-location sites then pooling that data and building a calibration model built on data from multiple sites will result in a calibration model that is more robust in new locations. 
\end{itemize}
