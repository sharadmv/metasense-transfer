...
\end{itemize}
\todo{(Add figure to explain meaning of different levels)}Notably in Level 0 and Level 3 benchmarks, the train and test data distribution have explicit overlap, whereas in Level 1 and 2, there is no explicit overlap. 
In each of these benchmarks, we measure
the mean-absolute error in the test set (although our models are trained with MSE, we evaluate with MAE to offer a closer comparison to the results from \citet{Zimmerman2018}).
...
measuring mean absolute error (MAE) \todo{(Our loss function is minimizing for MSE. Why are we using MAE?)} and the coefficient of variation of mean absolute error (CvMAE), to be consistent with results with \citet{Zimmerman2018},
...
In these results, our model on average has lower MAE and CvMAE in Level 1 and Level 2 benchmarks for the random forest model. We found that held out boards calibrated in this way had only a small drop in performance compared to when they were included in the global model.
